...
export default function FintrackSettingsPage() {
  ...
  async function saveCutoff(event: React.FormEvent) {
    event.preventDefault(); beginTask('cutoff-save', 'Menyimpan cutoff')
    try { const value = Number(cutoffDay || data?.month_cutoff_day || 1); const response = await fintrackRequest('/api/fintrack/preferences', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ month_cutoff_day: value }) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || 'Tanggal cutoff gagal disimpan'); await refreshData(); setCutoffDay(String(value)); setMessage({ kind: 'success', text: 'Tanggal cutoff laporan diperbarui.' }) } catch (error) { setMessage({ kind: 'error', text: error instanceof Error ? error.message : 'Tanggal cutoff gagal disimpan' }) } finally { endTask('cutoff-save') }
  }
  ...
  return <main className="ft-container"><Header user={user} eyebrow="Preferensi pribadi" title="Pengaturan" settings />{message && <p className={`ft-inline-message ft-${message.kind}`}>{message.text}</p>}
    ...
    <form className="ft-card ft-settings-card" onSubmit={saveCutoff} data-updating={isBusy('cutoff-save')}><div className="ft-theme-row"><span className="ft-account-icon"><CalendarBlank size={18} weight="fill" /></span><div><strong>Cutoff bulanan</strong><span>Menentukan awal periode ringkasan dan laporan.</span></div></div><div className="ft-field"><label htmlFor="month-cutoff">Periode dimulai setiap tanggal</label><select id="month-cutoff" className="ft-input" value={cutoffDay || String(data.month_cutoff_day)} onChange={(event) => setCutoffDay(event.target.value)}>{Array.from({ length: 28 }, (_, index) => index + 1).map((day) => <option key={day} value={day}>{day}</option>)}</select></div><p className="ft-hint">Contoh cutoff 25: laporan September mencakup 25 Agustus–24 September.</p><button className="ft-button ft-button-secondary" style={{ width: '100%' }}>Simpan cutoff</button></form>
