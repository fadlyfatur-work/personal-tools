...
import { Archive, ArrowDown, ArrowUp, Bank, Coins, CreditCard, DotsThreeVertical, Eye, EyeSlash, HandCoins, PencilSimple, PiggyBank, Plus, Tag, Trash, Wallet, X } from '@phosphor-icons/react'
...
export default function ManagePage() {
  ...
  return <main className="ft-container">
    ...
    {tab === 'wallets' ? <div className="ft-tab-panel">
      ...
      <section className="ft-manage-list">{displayedAccounts.map((account) => { const Icon = accountIcons[account.kind] || Wallet; const ownedIndex = owned.findIndex((item) => item.id === account.id); return <article className="ft-manage-row ft-wallet-manage-row" key={account.id} data-updating={isBusy(`wallet-${account.id}`) || isBusy('wallet-reorder')}><div className="ft-manage-identity"><span className="ft-account-icon"><Icon size={18} weight="fill" /></span><div className="ft-manage-copy"><strong>{account.name}</strong><span>{formatRupiah(account.current_balance)}{account.access_role !== 'owner' ? ', dibagikan' : account.include_in_net_worth === false ? ', tidak dihitung dalam aset' : ', dihitung dalam aset'}</span></div></div><details className="ft-action-menu"><summary aria-label={`Aksi untuk ${account.name}`}><DotsThreeVertical size={19} weight="bold" /></summary><div role="menu">{account.access_role === 'owner' && !autoSort && <><button type="button" role="menuitem" disabled={ownedIndex === 0} onClick={(event) => { void moveWallet(account, -1); event.currentTarget.closest('details')?.removeAttribute('open') }}><ArrowUp size={16} />Naikkan urutan</button><button type="button" role="menuitem" disabled={ownedIndex === owned.length - 1} onClick={(event) => { void moveWallet(account, 1); event.currentTarget.closest('details')?.removeAttribute('open') }}><ArrowDown size={16} />Turunkan urutan</button></>}{account.access_role === 'owner' && <button type="button" role="menuitem" onClick={(event) => { void updateWallet(account, { include_in_net_worth: account.include_in_net_worth === false }); event.currentTarget.closest('details')?.removeAttribute('open') }}>{account.include_in_net_worth === false ? <Eye size={16} /> : <EyeSlash size={16} />}{account.include_in_net_worth === false ? 'Sertakan dalam total aset' : 'Kecualikan dari total aset'}</button>}<button type="button" role="menuitem" onClick={(event) => { const name = window.prompt('Nama dompet', account.name)?.trim(); if (name && name !== account.name) void updateWallet(account, { name }); event.currentTarget.closest('details')?.removeAttribute('open') }}><PencilSimple size={16} />Ubah nama</button>{account.access_role === 'owner' && <button type="button" role="menuitem" className="danger" onClick={(event) => { void archiveWallet(account); event.currentTarget.closest('details')?.removeAttribute('open') }}><Archive size={16} />Arsipkan</button>}</div></details></article> })}</section>
