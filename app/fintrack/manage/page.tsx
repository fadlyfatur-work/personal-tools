...
import { Archive, ArrowDown, ArrowUp, Bank, Coins, CreditCard, Eye, EyeSlash, HandCoins, PencilSimple, PiggyBank, Plus, Tag, Trash, Wallet } from '@phosphor-icons/react'
...
export default function ManagePage() {
  const { data, user, accounts, categories, loading, setAccounts, setCategories, beginTask, endTask, isBusy } = useFintrack()
  ...
  async function moveWallet(account: Account, direction: -1 | 1) {
    ...
    const shared = accounts.filter((item) => item.access_role !== 'owner')
    const index = owned.findIndex((item) => item.id === account.id)
    ...
    setAccounts([...optimistic, ...shared])
    beginTask('wallet-reorder', 'Menyimpan urutan dompet')
    try {
      ...
      setMessage({ kind: 'error', text: error instanceof Error ? error.message : 'Urutan dompet gagal disimpan' })
    } finally { endTask('wallet-reorder') }
  }
  ...
  return (
    <main className="ft-container">
      ...
      {tab === 'wallets' ? <div className="ft-tab-panel">
        <section className="ft-manage-list">{accounts.map((account) => { const Icon = accountIcons[account.kind] || Wallet; const owned = accounts.filter((item) => item.access_role === 'owner'); const ownedIndex = owned.findIndex((item) => item.id === account.id); return <article className="ft-manage-row" key={account.id} data-updating={isBusy(`wallet-${account.id}`) || isBusy('wallet-reorder')}><span className="ft-account-icon"><Icon size={18} weight="fill" /></span><div><strong>{account.name}</strong><span>{formatRupiah(account.current_balance)}{account.access_role !== 'owner' ? ', dibagikan' : account.include_in_net_worth === false ? ', tidak dihitung dalam aset' : ', dihitung dalam aset'}</span></div><div className="ft-row ft-manage-actions">{account.access_role === 'owner' && <><button className="ft-icon-button" title="Naikkan urutan" disabled={ownedIndex === 0} onClick={() => moveWallet(account, -1)}><ArrowUp size={15} /></button><button className="ft-icon-button" title="Turunkan urutan" disabled={ownedIndex === owned.length - 1} onClick={() => moveWallet(account, 1)}><ArrowDown size={15} /></button><button className="ft-icon-button" title={account.include_in_net_worth === false ? 'Sertakan dalam kekayaan' : 'Kecualikan dari kekayaan'} onClick={() => updateWallet(account, { include_in_net_worth: account.include_in_net_worth === false })}>{account.include_in_net_worth === false ? <Eye size={16} /> : <EyeSlash size={16} />}</button></>}<button className="ft-icon-button" title="Ubah nama" onClick={() => { const name = window.prompt('Nama dompet', account.name)?.trim(); if (name && name !== account.name) void updateWallet(account, { name }) }}><PencilSimple size={16} /></button>{account.access_role === 'owner' && <button className="ft-icon-button" title="Arsipkan" onClick={() => archiveWallet(account)}><Archive size={16} /></button>}</div></article> })}</section>
