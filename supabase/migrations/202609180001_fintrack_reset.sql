...
begin
  ...
  if p_from_account_id is not null then
    v_from_delta := case when p_type = 'expense' and v_from_class = 'liability' then p_amount else -p_amount end;
  ...
  if p_to_account_id is not null then
    v_to_delta := case when p_type = 'income' and v_to_class = 'liability' then -p_amount else p_amount end;
    insert into public.fintrack_transaction_entries (transaction_id, account_id, amount_delta)
...
begin
  ...
  v_new := public.fintrack_create_transaction(
    ...
    p_category_id, p_note, p_transaction_date
  );
